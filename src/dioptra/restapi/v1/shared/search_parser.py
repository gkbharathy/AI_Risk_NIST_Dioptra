...
def _define_query_grammar() -> pp.ParserElement:
    ...
    escape = pp.Literal("\\") + pp.Word("\\?*\"'", exact=1)
    ...
    sgl_quoted_words = pp.Combine(
        (escape | pp.Word(pp.printables + " ", exclude_chars="\\?*'"))[1, ...]
    )
...
def parse_search_text(search_text: str) -> list[dict]:
    ...
    parsed_search = DIOPTRA_QUERY_GRAMMAR.parse_string(search_text).as_list()
    formatted_result = []
...
def construct_sql_search_value(search_term: list[str]) -> str:
    """
...
def construct_sql_query_filters(search_string: str, searchable_fields: dict[str, Any]):
    ...
    try:
        ...
        raise SearchParseError from None
    ...
    for search_term in parsed_search_terms:
        ...
        if field is None:
            filter = or_(col.like(value) for col in searchable_fields.values())
        elif field in searchable_fields:
            filter = searchable_fields[field].like(value)
        else:
